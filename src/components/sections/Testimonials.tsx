      ...
      <Container>
        <div className='mx-auto max-w-2xl text-center'>
          ...
          <h2 className='mt-4 font-display text-4xl font-medium leading-tight tracking-tightest text-ink sm:text-5xl text-balance'>
            Lo que dicen quienes recetan Vitalis.
          </h2>
        </div>
        ...
        <ul className='mt-16 grid gap-8 md:grid-cols-2'>
          {TESTIMONIALS.map(t => (
            <li
              key={t.name}
              className='flex h-full flex-col justify-between rounded-3xl border border-ink/8 bg-surface-bone/40 p-10 hover-lift'
            >
              <blockquote>
                <svg viewBox='0 0 32 24' aria-hidden='true' className='size-8 text-accent' fill='currentColor'>
                  <path d='M0 24V14C0 6.3 4.6 1 12 0v5C8.4 6 6 8.4 6 13h6v11H0Zm20 0V14c0-7.7 4.6-13 12-14v5c-3.6 1-6 3.4-6 8h6v11H20Z' />
                </svg>
                <p className='mt-6 font-display text-2xl font-medium leading-snug tracking-tight text-ink text-pretty'>
                  «{t.quote}»
                </p>
              </blockquote>
